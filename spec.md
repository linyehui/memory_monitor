...
- **依赖**: 仅使用系统内置命令 (vm_stat, sysctl, ps, plutil)
- **颜色**: ANSI 转义码实现终端颜色高亮
...
3. **get_process_name()**: 获取进程的本地化名称
   - 从 .app 应用的 Info.plist 读取 CFBundleName
   - 使用 plutil 提取，正确处理中文等非 ASCII 字符
   - 自动根据系统语言返回本地化名称（与 Activity Monitor 行为一致）
   - 对于非 .app 应用，回退到使用可执行文件的 basename

4. **format_duration()**: 格式化运行时长
   - 支持秒、分钟、小时、天的显示
...
- ✅ 系统内存监控 (RAM + Swap)
- ✅ 进程内存监控 (RSS + VSZ)
- ✅ 进程本地化名称显示（与 Activity Monitor 一致）
- ✅ 增量计算和红色警告
- ✅ 固定位置刷新显示
- ✅ PID 参数验证
- ✅ 时长格式化显示
